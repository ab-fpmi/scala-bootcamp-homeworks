...
object AsyncHomework extends App {
  ...
  val names = for {
    ...
    names <- Future.sequence(links.map(fetchServerName))
  } yield names

  names.map(_.flatten.distinct.sorted.foreach(println)).recover {
    case e =>
      println(e)
      println("=====")
  ...
  private def fetchPageBody(url: String): Future[String] = {
    ...
    Future {
      val source = Source.fromURL(url)(Codec("ISO-8859-1"))
      try {
        source.mkString
      } finally {
        source.close()
