...
object Exercise3 extends App {
  ...
  case class EntityDied(entityId: String)
  ...
  final class EntityRouterActor(
    ...
    private def working: Receive = {
      ...
      case EntityDied(entityId) =>
        entityRefs = entityRefs - entityId

      case cmd =>
        val entityId = extractEntityId(cmd)
        val ref = entityRefs.getOrElse(entityId, startEntity(entityId))
        ref forward cmd
    }
    ...
    private def startEntity(entityId: String): ActorRef = {
      val ref = system.actorOf(MyEntity.props, ActorNameSanitizer.sanitize(entityId))
      context.watchWith(ref, EntityDied(entityId))
      entityRefs = entityRefs + (entityId -> ref)
      ref
    }

    private def poisonAll(): Unit =
  ...
  object MyEntity {
    val props: Props = Props[MyEntity]()
  }
